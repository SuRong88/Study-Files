...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e2298bd3dccb4c46b90c276255e54bf0~tplv-k3u1fbpfcp-zoom-1.image)
...
> 由于在面试中，三次握手是被问的最频繁的面试题，所以本次我们从**面试**的角度来讲解三次握手
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e251846fdc6f4ff2b363e5aa3b32afd5~tplv-k3u1fbpfcp-zoom-1.image)

这里特别需要主要的就是****TIME_WAIT****这个状态了，这个是面试的高频考点，就是要理解，为什么客户端发送 ACK 之后不直接关闭，而是要等一阵子才关闭。这其中的原因就是，要确保服务器是否已经收到了我们的 ACK 报文，如果没有收到的话，服务器会重新发 FIN 报文给客户端，客户端再次收到 ACK（这里是FIN 吧） 报文之后，就知道之前的 ACK 报文丢失了，然后再次发送 ACK 报文。

至于 TIME_WAIT 持续的时间至少是一个报文的来回时间。一般会设置一个计时，如果过了这个计时没有再次收到 FIN 报文，则代表对方成功接收 ACK 报文，此时处于 CLOSED 状态。
...
![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4f0c8a39a54e49a7aae2ef925dd0e5ac~tplv-k3u1fbpfcp-zoom-1.image)

##  3.个人总结

三次握手

发送信息SYN J,确认收到信息ACK J+1。SYN用来发送，ACK于确认收到，ACK的序列号是SYN的序列号+1。在第二次握手的时候，服务端发送了SYN和ACK，简单理解为服务端展示了他的发送能力和接受能力。

四次挥手

发送信息FIN J,确认收到信息ACK J+1；发送信息FIN K,确认收到信息ACK K+1。

重点在于知道为什么第四次挥手需要有一个TIME WAIT，还有为什么TIME WAIT的值至少是一个报文来回的时间。
